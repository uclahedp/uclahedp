...
def spectrum(wavelength, mode='collective', Te=None, Ti=None,
     ...
     #************************************************************************
         ...
         xe = np.sqrt(2)*wdoppler_e/(k*vTe)
         xi=np.sqrt(2)/np.outer(1/vTi, wdoppler_i/k) #Schaeffer 5.21
...
if __name__ == "__main__":
     ...
     k, spectral_dist = spectrum(wavelength, mode='collective',
               Te=10e-3, Ti=10e-3, electron_v=electron_v, ion_v = ion_v,
               probe_n = probe_n, scatter_n=scatter_n, B0=B0,
               pol_n = None,
               ion_fract=np.array([1.0]), ion_Z=np.array([5]),
               ion_Mu=np.array([12]), ne=ne, probe_wavelength=w,
